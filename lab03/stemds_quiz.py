...
class _QuestionWidget:
    ...
    def _build(self):
        ...
        for i, ans in enumerate(answers):
            btn = widgets.Button(
                ...
                layout      = widgets.Layout(width="96%", min_height="38px",
                                             margin="2px 0"),
                style       = {"button_color": _C["light_grey"], "font_weight": "500"}
            )
            btn._ans_index = i
            if not self.locked:
    ...
    def _on_click(self, btn):
        ...
        btn.style.button_color = _C["green"] if is_correct else _C["red"]
        btn.style.text_color   = _C["white"]
        if not is_correct:
            btn.disabled = True
        ...
        if self.locked:
            for b in self._answer_btns:
                b.disabled = True
                if self.data["answers"][b._ans_index]["correct"]:
                    b.style.button_color = _C["pale_green"]
...
def open_quiz(questions_source, name, user,
              quiz_id=None, max_attempts=None):
    """
    ...
    if isinstance(questions_source, str):
        ...
        questions = list(questions_source)
