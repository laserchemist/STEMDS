...
def ptrend(tbl, datec, datac, label=None, color=None, fmtdate="%b-%Y"):
    """Takes Datascience Table and plots time trend, returns number of days."""

    # Input data
    datetrend = tbl.column(datec).astype("datetime64[s]")
    ...
    # Plot with or without a specified color
    if color is None:
        plt.plot(datetrend, data, label=label)
    else:
        plt.plot(datetrend, data, color=color, label=label)
