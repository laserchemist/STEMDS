...
lab_submit.py
=============
Place this file in the course root directory (alongside notebook_style.py).
It is importable from any lab subdirectory via the sys.path fix in labXX_init.py.

Student notebook — final cell:
────────────────────────────────────────────────────────
    from lab_submit import submit_lab
    submit_lab(name, user, correct, questions, lab_id="Lab05")
────────────────────────────────────────────────────────

Write targets (tried in order):
  1. Google Sheets  — primary, works for all hub users
  2. Home fallback  — ~/submissions_local.json (always writable)

NOTE: /home/jovyan/shared-readwrite/ is admin-only on this hub.
      /home/jovyan/shared/ is read-only for all users.
      Google Sheets is therefore the only reliable shared write target.
      The credentials file must be readable by all users:
        /home/jovyan/shared/nordic-knowledge-6598a7fdb7c8.json
"""
...
LAB_ID = "unknown"   # always pass lab_id= explicitly in submit_lab()
...
def submit_lab(name, user, correct, questions, lab_id=LAB_ID):
    """
    Display score summary and Submit button.

        from lab_submit import submit_lab
        submit_lab(name, user, correct, questions, lab_id="Lab05")
    """
    perc_correct = correct / len(questions) * 100
    msg = "nice work!" if perc_correct >= 80 else \
          "look over your work again, seek help, some errors!!!"
